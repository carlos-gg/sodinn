...
def predict_mlar(mode, model, cube, angle_list, fwhm, in_ann, out_ann,
    ...
    if verbose:
        ...
        print('Grabbing MLAR samples per annulus')
    ...
    if verbose:
        timing(starttime)
        print('Generating a prediction on each MLAR sample')
    ...
    elif hasattr(model, 'name'):  # Neural network
...
def get_mlar_patches(i, fwhm, angle_list, patch_size, collapse_func, scaling,
    ...
    for i in range(num_patches):
        patches_frame.append(cube_crop_frames(np.array(cube_residuals),
                                              patch_size, xy=(int(xx[i]),
                                                              int(yy[i])),
